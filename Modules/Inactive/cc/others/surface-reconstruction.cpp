...
void print_usage() {
  std::cout << "usage: surface-reconstructor [-s|-v|-r]\n"
            << " -s smooth points using mls, dump to scan-mls.pcd\n"
            << " -v visualize smoothed points, load from scan-mls.pcd\n"
            << " -r smooth and reconstruct surface, load from scan-mls.pcd dump to mesh.vtk\n";
}
...
pcl::PolygonMesh recons2(pcl::PointCloud<pcl::PointNormal>::Ptr cloud, std::string outfile) {
